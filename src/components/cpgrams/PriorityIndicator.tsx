    ...
    <div className="space-y-2">
      ...
      {!compact && (
        <details className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs">
          <summary className="cursor-pointer font-semibold text-foreground">Why this priority?</summary>
          {priority.priority_reasons.length ? (
            <ul className="mt-2 list-disc space-y-1 pl-4 text-muted-foreground">
              {priority.priority_reasons.map((reason) => <li key={reason}>{reason}</li>)}
            </ul>
          ) : (
            <p className="mt-2 text-muted-foreground">No dynamic escalation factor currently applies.</p>
          )}
