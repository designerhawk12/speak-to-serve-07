  ...
  return (
    <div className="space-y-4">
      {requests.map((request) => {
        ...
        const optionalCount = requestItems.filter((item) => !item.is_required).length;
        return (
          <section
            key={request.id}
            className="space-y-3 rounded-lg border border-warning/35 bg-warning-surface p-4"
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{request.reason}</h3>
                <p className="mt-1 text-sm text-muted-foreground">
                  {progress.required} document{progress.required === 1 ? "" : "s"} required ·{" "}
                  {progress.supplied} of {progress.required} required documents uploaded
                </p>
                {optionalCount > 0 && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    {optionalCount} optional item{optionalCount === 1 ? "" : "s"} may also be
                    uploaded.
                  </p>
                )}
              </div>
              {request.fulfilled_at && (
                <span className="text-xs font-semibold text-success">Request completed</span>
              )}
            </div>
            <ul className="space-y-3">
              {requestItems.map((item) => {
                const attachedDocument = item.document_id
                  ? documentsById.get(item.document_id)
                  : undefined;
                const selectedFile = selectedFiles[item.id];
                const isCurrentUpload =
                  upload.isPending && upload.variables?.requestItemId === item.id;
                return (
                  <li key={item.id} className="rounded-md border border-border bg-background p-3">
                    <div className="flex gap-2">
                      <CheckCircle2
                        className={`mt-0.5 size-4 shrink-0 ${attachedDocument ? "text-success" : "text-muted-foreground"}`}
                        aria-hidden
                      />
                      <div className="min-w-0 flex-1 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            {attachedDocument ? "Uploaded" : "Not uploaded"}
                          </span>
                        </div>
                        {item.description && (
                        ...
                        {attachedDocument ? (
                          <p className="text-sm text-success">
                            Supplied: {attachedDocument.file_name}
                          </p>
                        ) : (
                          <div className="flex flex-wrap items-center gap-2">
                            <Input
                              type="file"
                              aria-label={`Choose file for ${item.label}`}
                            ...
                            <Button
                              ...
                              onClick={() =>
                                selectedFile &&
                                upload.mutate({ file: selectedFile, requestItemId: item.id })
                              }
                            >
                              <Upload className="size-4" aria-hidden />
                              {isCurrentUpload ? "Uploading" : "Upload"}
                            </Button>
                          </div>
                        )}
                      </div>
            ...
            {progress.required > 0 &&
              progress.supplied === progress.required &&
              !request.fulfilled_at && (
                <p className="rounded-md bg-success-surface p-3 text-sm text-success">
                  All required documents have been uploaded. The case will update when the request
